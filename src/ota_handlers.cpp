...
esp_err_t ota_spiffs_handler(httpd_req_t* req) {
    ...
    ESP_LOGI(TAG, "Inizio OTA SPIFFS, dimensione: %d bytes, partizione: %u bytes",
             req->content_len, (unsigned int)part->size);
    ...
    if (!buf) {
        buf = (uint8_t*)malloc(CHUNK);
        if (!buf) {
            storage_remount(); // Tenta di rimontare
            httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Alloc buffer fallita");
            return ESP_FAIL;
        }
    ...
    while (remaining > 0) {
        ...
        if (need_end > erased_until) {
            ...
            while (erased_until < erase_to) {
                ...
                if (esp_partition_erase_range(part, aligned_start, aligned_len) != ESP_OK) {
                    ESP_LOGE(TAG, "Erase fallito a offset 0x%X, len 0x%X", (unsigned int)aligned_start, (unsigned int)aligned_len);
                    free(buf);
                    storage_remount();
                ...
                // Log progresso erase
                ESP_LOGD(TAG, "Erased fino a 0x%X", (unsigned int)erased_until);
        ...
        if (esp_partition_write(part, offset, buf, r) != ESP_OK) {
            ESP_LOGE(TAG, "Scrittura SPIFFS fallita a offset 0x%X", (unsigned int)offset);
            free(buf);
            storage_remount();
        ...
        if (offset % (32 * 1024) == 0) {
            ESP_LOGI(TAG, "SPIFFS progresso: %u/%d bytes (%.1f%%)",
                     (unsigned int)offset, req->content_len, 100.0f * offset / req->content_len);
        }
    ...
    if (offset < part->size) {
        ...
        if (tail_start < part->size) {
            ...
            ESP_LOGI(TAG, "Pulizia coda da 0x%X, lunghezza 0x%X", (unsigned int)tail_start, (unsigned int)tail_len);
